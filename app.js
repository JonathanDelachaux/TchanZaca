...
function gallerySceneFromUV(uv){
  if(!uv)return null;
  if(uv.y<0.35||uv.y>0.62)return null;

  let best=null;
  for(const hotspot of GALLERY_HOTSPOTS){
    const direct=circularDistance(uv.x,hotspot.x);
    const mirrored=circularDistance(1-uv.x,hotspot.x);
    const distance=Math.min(direct,mirrored);
    if(!best||distance<best.distance)best={scene:hotspot.scene,distance};
  }
  return best&&best.distance<0.065?best.scene:null;
}

function openGalleryArtworkAt(clientX,clientY){
  if(mode!=="gallery"||!sphere)return false;
  ...
  const sceneIndex=gallerySceneFromUV(hit.uv);
...
prev.addEventListener("click",()=>mode==="gallery"?loadScene(SCENES.length-1):loadScene(index-1));galleryBtn.addEventListener("click",()=>loadGallery());next.addEventListener("click",()=>mode==="gallery"?loadScene(0):loadScene(index+1));
renderer.domElement.addEventListener("pointerdown",e=>{down=true;px=e.clientX;py=e.clientY;pointerStartX=e.clientX;pointerStartY=e.clientY;pointerMoved=false;renderer.domElement.setPointerCapture(e.pointerId);if(video?.paused)video.play().catch(()=>{})});
renderer.domElement.addEventListener("pointermove",e=>{if(!down)return;const dx=e.clientX-px,dy=e.clientY-py;if(Math.hypot(e.clientX-pointerStartX,e.clientY-pointerStartY)>8)pointerMoved=true;px=e.clientX;py=e.clientY;yaw-=dx*.0045;pitch-=dy*.0045;pitch=THREE.MathUtils.clamp(pitch,-Math.PI/2.05,Math.PI/2.05)});
renderer.domElement.addEventListener("pointerup",e=>{const wasClick=!pointerMoved&&Math.hypot(e.clientX-pointerStartX,e.clientY-pointerStartY)<=8;down=false;try{renderer.domElement.releasePointerCapture(e.pointerId)}catch{}if(wasClick)openGalleryArtworkAt(e.clientX,e.clientY)});
renderer.domElement.addEventListener("pointercancel",()=>{down=false;pointerMoved=false});
...
function animate(){requestAnimationFrame(animate);updateCamera();renderer.render(scene,camera)}animate();loadGallery();
